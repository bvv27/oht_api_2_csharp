...
namespace oht.tests
{
    [TestFixture, Description("Tests CreateProofreadingProjectSourceAndTarget")]
    public class CreateProofreadingProjectSourceAndTarget
    {

        private const string ExpectedJsonResultOk = "{\"status\":{\"code\":0,\"msg\":\"ok\"},\"results\":{\"project_id\":\"808113\",\"wordcount\":3,\"credits\":21},\"errors\":[]}";
        ...
        [Test, Description("Tests CreateProofreadingProjectSourceAndTarget")]
        public void Test()
        {
            ...
            var result = ohtapi.CreateProofreadingProjectSourceAndTarget("en-us", "ru-ru", "rsc-560e7ea4650793-27822858", "rsc-560ea011277cb3-18000700");

            Assert.IsNotNull(result);
            Assert.AreNotEqual(0, result.Status.Code);
            Assert.AreNotEqual(-1, result.Status.Code);

            var provider = Substitute.For<ICreateProofreadingProjectSourceAndTargetProvider>();
            ohtapi.CreateProofreadingProjectSourceAndTargetProvider = provider;
            ...
            result = ohtapi.CreateProofreadingProjectSourceAndTarget(string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
